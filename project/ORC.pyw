from tkinter import *
import random
colors = ['blue', 'red', 'yellow', 'purple', 'green', 'pink', 'orange', 'grey', 'light blue', 'tomato','fuchsia']
...
def roll(x):
    ...
    if x == 'greataxe':
        Attack1_Entry.insert(END, random.randint(6,25))
        Attack1_Entry2.insert(END, random.randint(4,15))

    if x == 'javelin':
        Attack2_Entry.insert(END, random.randint(6,25))
        Attack2_Entry2.insert(END, random.randint(4,9))
    
    if x == 'orc_str':
        STREntry.insert(END, random.randint(4, 23))

    if x == 'orc_dex':
        DEXEntry.insert(END, random.randint(2,21))

    if x == 'orc_con':
        CONEntry.insert(END, random.randint(3,23))
...
main = Tk()
main.title("Orc")
image = PhotoImage(file='Orc.png')
OrcLabel =  Label(main, image=image,bd=2, relief="solid")
OrcLabel.grid()

miscEntry = Entry(main)
miscEntry.place(x=130, y=35)

InitEntry = Entry(main, width=2)
InitEntry.place(x=270, y=35)

InitEntry.insert(0, random.randint(2,21))


HPLabel = Label(main, text="Current HP:").place(x=90, y=100)
HPEntry = Entry(main, width=2)
HPEntry.place(x=160, y=100)
HPEntry.insert(END, '15')
#ATTACK 1
Frame1 = Frame(main, bg=random.choice(colors))
Frame1.grid(row=3, column=0)

Attack1_Button = Button(Frame1, text='Greataxe ', command=lambda:roll('greataxe'))
...
#ATTACK 2
Attack2_Button = Button(Frame1, text='  Javelin  ', command=lambda:roll('javelin'))
