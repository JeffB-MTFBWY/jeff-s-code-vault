...
def roll(x):
    ...
    if x == 'scimitar':
        Attack1_Entry.insert(END, random.randint(5,24))
        Attack1_Entry2.insert(END, random.randint(3,8))

    if x == 's_bow':
        Attack2_Entry.insert(END, random.randint(5,24))
        Attack2_Entry2.insert(END, random.randint(3,8))
    
    if x == 'orc_str':
        STREntry.insert(END, random.randint(1, 19))

    if x == 'orc_dex':
        DEXEntry.insert(END, random.randint(2,22))

    if x == 'orc_con':
        CONEntry.insert(END, random.randint(1,20))

    if x == 'orc_int':
        INTEntry.insert(END, random.randint(1,20))

    if x == 'orc_wis':
        WISEntry.insert(END, random.randint(1,19))

    if x == 'orc_cha':
        CHAEntry.insert(END, random.randint(1,19))
        

main = Tk()
main.title("Goblin")
image = PhotoImage(file='Goblin.png')
OrcLabel =  Label(main, image=image,bd=2, relief="solid")
OrcLabel.grid()

miscEntry = Entry(main)
miscEntry.place(x=130, y=20)

InitEntry = Entry(main, width=2)
InitEntry.place(x=270, y=20)

InitEntry.insert(0, random.randint(3,22))


HPLabel = Label(main, text="Current HP:").place(x=90, y=85)
HPEntry = Entry(main, width=2)
HPEntry.place(x=160, y=85)
HPEntry.insert(END, '7')

#ATTACK 1
Frame1 = Frame(main, bg=random.choice(colors))
Frame1.grid(row=3, column=0)

Attack1_Button = Button(Frame1, text='Scimitar ', command=lambda:roll('scimitar'))
...
#ATTACK 2
Attack2_Button = Button(Frame1, text='Short bow  ', command=lambda:roll('s_bow'))
